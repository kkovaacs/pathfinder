...
impl StateUpdate {
    ...
    pub fn state_diff_commitment(&self) -> StateDiffCommitment {
        StateDiffCommitment(
            poseidon_hash_many(&[
                // state_diff_version
                FieldElement::ZERO,
                self.hash_of_deployed_contracts(),
                self.hash_of_declared_classes(),
                self.hash_of_old_declared_classes(),
                // number_of_DA_modes
                FieldElement::ONE,
                // DA_mode_0
                FieldElement::ZERO,
                self.hash_of_storage_domain_state_diff(),
            ])
            .into(),
        )
    }

    fn hash_of_deployed_contracts(&self) -> FieldElement {
    ...
    fn hash_of_declared_classes(&self) -> FieldElement {
    ...
    fn hash_of_old_declared_classes(&self) -> FieldElement {
    ...
    fn hash_of_storage_domain_state_diff(&self) -> FieldElement {
